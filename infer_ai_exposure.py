...
def calculate_ai_exposure(title: str, category: str, education: str, 
                         outlook: int, jobs: int) -> Tuple[int, str]:
    ...
    if any(kw in title_lower for kw in [
        "software", "programmer", "data", "analyst", "accountant",
        "clerk", "secretary", "administrative", "bookkeeper",
        "telemarketer", "cashier", "computer", "information"
    ]):
        base_score += 2
    
    # Medium-high AI exposure (+1)
    elif any(kw in title_lower for kw in [
        "manager", "engineer", "designer", "writer", "editor",
        "marketing", "finance", "planner", "consultant", "lawyer"
    ]):
        base_score += 1
    
    # Low AI exposure (-1)
    elif any(kw in title_lower for kw in [
        "cleaner", "driver", "mechanic", "repair", "construction",
        "agriculture", "farmer", "fisher", "care", "nurse"
    ]):
        base_score -= 1
    
    # Physical/manual work keywords (-2)
    if any(kw in title_lower for kw in [
        "hand", "manual", "physical", "craft", "welder", "carpenter",
        "plumber", "electrician", "mason", "painter", "hairdresser"
    ]):
        base_score -= 2
...
def generate_rationale(title: str, category: str, education: str, score: int) -> str:
    ...
    if score >= 8:
        return (f"This {cat_name} occupation involves extensive digital knowledge work "
        ...
        return (f"This {cat_name} occupation has significant AI exposure due to substantial "
        ...
        return (f"This {cat_name} occupation has moderate AI exposure. While some tasks "
        ...
        return (f"This {cat_name} occupation has low AI exposure. The work is primarily "
        ...
        return (f"This {cat_name} occupation has minimal AI exposure. The work is almost "
...
def load_site_data() -> List[dict]:
    ...
    if not os.path.exists("site/data.json"):
        raise FileNotFoundError("site/data.json not found. Run build_site_data.py first.")
