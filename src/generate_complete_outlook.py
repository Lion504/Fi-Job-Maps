...
def load_existing_extras() -> Dict[str, dict]:
    ...
    if not os.path.exists("data/extras.csv"):
        return {}

    extras = {}
    with open("data/extras.csv") as f:
...
def load_occupations() -> List[Tuple[str, str]]:
    ...
    if not os.path.exists("data/occupations.json"):
        raise FileNotFoundError(
            "occupations.json not found. Run fetch_statfin.py first."
        )
...
def infer_outlook(
    code: str, title: str, extras: Dict[str, dict], category_avg: Dict[str, dict]
) -> Tuple[str, str]:
    ...
    if not code.endswith("."):
        ...
        if dotted_code in extras:
            return (
                extras[dotted_code].get("outlook_pct", ""),
    ...
    if code.endswith("."):
        ...
        if base_code in extras:
            return (
                extras[base_code].get("outlook_pct", ""),
    ...
    if category in category_avg:
        return (
            str(category_avg[category]["outlook_pct"]),
...
def main():
    ...
    for code, title in occupations:
        ...
        if "(Level 4)" in title or "(Level 5)" in title:
            if code in extras:
                ...
                # Infer outlook
                outlook_pct, outlook_desc = infer_outlook(
                    code, title, extras, category_avg
                )
    ...
    output_file = "data/extras_complete.csv"
